package cn.lanink.gamecore.mailapi.utils;

import cn.lanink.gamecore.mailapi.NukkitLoad;
    ...
    private static int checkAndDownload(int retry) {
        ...
        if (plugin != null) {
            if (!VersionUtils.checkMinimumVersion(plugin, ACTUAL_MINIMUM_GAME_CORE_VERSION)) {
                NukkitLoad.getInstance().getLogger().warning("MemoriesOfTime-GameCore依赖版本太低！正在尝试更新版本...");
                ...
                if (file != null) {
                    ...
                    NukkitLoad.getInstance().getLogger().error("删除旧版本失败！请手动删除！");
                }
            }
        }

        if (plugin == null || plugin.isDisabled()) {
            NukkitLoad.getInstance().getLogger().info("尝试从 " + url + " 下载 MemoriesOfTime-GameCore 中...");
            ...
            try {
                ...
                download(url, file, (len, fullLength) -> {
                    ...
                    if (d - last.get() > 15) { // 每15%提示一次
                        NukkitLoad.getInstance().getLogger().info("已下载：" + d + "%");
                        last.set(d);
                    }
                });
            } catch (Exception e) {
                NukkitLoad.getInstance().getLogger().error("MemoriesOfTime-GameCore依赖下载失败！");
                return checkAndDownload(++retry);
            }

            NukkitLoad.getInstance().getLogger().info("MemoriesOfTime-GameCore依赖下载成功！");
